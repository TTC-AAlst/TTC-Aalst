...
public class PlayerNotesControllerTests : IntegrationTestBase
{
    ...
    public async Task Save_CreatesTheNote()
    {
        ...
        var response = await client.PostAsJsonAsync("api/playernotes", NewRequest("Speelt zeer defensief"));

        response.EnsureSuccessStatusCode();
        var saved = await response.Content.ReadFromJsonAsync<PlayerNote>();
    ...
    {
        using var client = CreateClientFor(PlayerId);
        (await client.PostAsJsonAsync("api/playernotes", NewRequest("Eerste versie"))).EnsureSuccessStatusCode();

        (await client.PostAsJsonAsync("api/playernotes", NewRequest("Tweede versie"))).EnsureSuccessStatusCode();
    ...
    {
        using var client = CreateClientFor(PlayerId);
        (await client.PostAsJsonAsync("api/playernotes", NewRequest("Backhand is zwak"))).EnsureSuccessStatusCode();

        (await client.PostAsJsonAsync("api/playernotes", NewRequest("   "))).EnsureSuccessStatusCode();
    ...
    public async Task Save_WithoutOpponent_IsBadRequest()
    {
        ...
        var response = await client.PostAsJsonAsync("api/playernotes", NewRequest("Geen tegenstander", uniqueIndex: 0));
    ...
    public async Task GetMine_ReturnsOnlyMyOwnNotes()
    {
        ...
        (await mine.PostAsJsonAsync("api/playernotes", NewRequest("Mijn notitie"))).EnsureSuccessStatusCode();
        (await theirs.PostAsJsonAsync("api/playernotes", NewRequest("Hun notitie"))).EnsureSuccessStatusCode();

        var notes = await mine.GetFromJsonAsync<PlayerNote[]>("api/playernotes");
    ...
    public async Task Delete_OfSomebodyElsesNote_IsNotFound()
    {
        using var mine = CreateClientFor(PlayerId);
        using var theirs = CreateClientFor(OtherPlayerId);
        var response = await theirs.PostAsJsonAsync("api/playernotes", NewRequest("Hun notitie"));
        response.EnsureSuccessStatusCode();
        var theirNote = await response.Content.ReadFromJsonAsync<PlayerNote>();
    ...
    {
        using var client = CreateClientFor(PlayerId);
        var response = await client.PostAsJsonAsync("api/playernotes", NewRequest("Weg ermee"));
        response.EnsureSuccessStatusCode();
        var note = await response.Content.ReadFromJsonAsync<PlayerNote>();
